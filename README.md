...
평소 ref를 사용 해본 적이 거의 없어 해당 부분 구현에 어려움을 겪었습니다. 컴포넌트에 ref를 할당 해주면 바로 해당 컴포넌트를 참조 가능할 줄 줄 알았지만, 초기에 들어간 null값만 참조 되었습니다. 오랜 시간 동안 고민해보았고, 그 결과 컴포넌트를 재렌더링 하기 위해선, state가 바뀌거나 props가 바뀌어야 한다는 사실을 알 수 있었습니다. 따라서 ref에 초기값을 null로 설정 해두고 랜더링 조건이 발동되지 않아 ref는 계속 null만 참조한 것이었습니다. 이번 과제를 통해 ref와, 컴포넌트의 렌더링 조건에 대해서 확실하게 알 수 있는 계기가 된 것 같습니다.
